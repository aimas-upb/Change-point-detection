...
class StatsTimeBetweenActivations(Feature):
    def __init__(self, mode):
        self.name = 'Percentile stats for time between activations ' + mode
        self.mode = mode
    ...
    def get_result(self, window):
        ...
        return np.array([np.mean(result), np.std(result), np.percentile(result, 25), np.percentile(result, 50),
